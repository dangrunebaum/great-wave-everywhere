...
/**
 * Fetches all images associated with a given word.
 * Used to render images for a specific word.
 */
export async function fetchImagesForWord(word) {
  const ref = doc(db, "words", word.toLowerCase());
  const snap = await getDoc(ref);
  console.log("fetchImagesForWord:", {
    word,
    ref,
    exists: snap.exists(),
    data: snap.data(),
